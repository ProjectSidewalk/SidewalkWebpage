...
1. Email Mikey (saugstad@cs.washington.edu) and ask for two database dumps and a `docker-compose.override.yml` file with the API keys. If you are not part of our team, you'll have to create API keys for [Google Maps](https://developers.google.com/maps/documentation/javascript/get-api-key) and [Mapbox](https://docs.mapbox.com/help/getting-started/access-tokens/) yourself.
1. Add the `docker-compose.override.yml` file to the root SidewalkWebpage directory.
1. If your computer has an Apple Silicon (M1 or M2) chip, then you should uncomment the `platform` line in the `docker-compose.override.yml`.
1. Modify the `SIDEWALK_CITY_ID` line in the `docker-compose.override.yml` to use the ID of the appropriate city, listed [here](https://github.com/ProjectSidewalk/SidewalkWebpage/wiki/Docker-Troubleshooting#first-heres-a-table-that-youll-reference-when-setting-up-your-dev-env) (it's the city that matches your database dump, so check the name of the db dump file).
1. Modify the `DATABASE_USER` line in the `docker-compose.override.yml`, replacing "sidewalk" with the username from the table [linked above](https://github.com/ProjectSidewalk/SidewalkWebpage/wiki/Docker-Troubleshooting#first-heres-a-table-that-youll-reference-when-setting-up-your-dev-env).
