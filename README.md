...
#### Windows with WSL2 (If you have Windows Home, you have to follow these instructions)
If you are using Windows, is is recommended to use WSL2. This will massively increase your compile times from minutes/seconds to milliseconds. You will also have to choose and Linux Distibution you want to use. Follow these intructions to download it https://docs.microsoft.com/en-us/windows/wsl/install-win10.

After that, go into Docker, click the setting wheel on the top and General. Enable "Use the WSL 2 based engine". Then go to Resources > WSL Integration and enable integration with the new distro you just installed under "Enable integration with additional distros:".
Then go into your desired location in your Linux terminal (the distribution you just downloaded from the app store) and run `git clone https://github.com/ProjectSidewalk/SidewalkWebpage.git`.
  
If you want to transfer files to and from your Linux distro and your Windows workspace, go to file explorer. Then on the top where the file paths are, empty it and type `\\wsl$`. Find your Linux distro and you can right click on it to pin it to your File Explorer. Then you can freely tranfer files to and from it. This is going to be used for moving your API Keys and database dump.
  
#### Windows with Hyper-V

If you are using Windows Pro or higher and do not want to use WSL2, continue with the setup. Otherwise follow the Windows with WSL2 tutorial,

You might also need to install `make` in order to run the dev environment. We recommend using Powershell as the terminal.
...
On Windows with Hyper-V, we recommend [Windows Powershell](https://docs.microsoft.com/en-us/powershell/scripting/overview?view=powershell-7) (built in to Win10). On Mac, use the basic terminal or, even better, [iTerm2](https://www.iterm2.com/). Users who have WLS2 installed should just use the terminal for the Linux distro.
  
1. Email Mikey (michaelssaugstad@gmail.com) and ask for the two API key files and a database dump. You will put the API key files into the root directory of the project. Rename the database dump `sidewalk-dump` and put it in the `SidewalkWebpage/db` directory (other files in this dir include `init.sh` and `schema.sql`, for example). If you are using WSL2, you can transfer files through the file explorer. Read Windows with WSL2.
  
1. If the database dump is for a city other than DC, modify the `SIDEWALK_CITY_ID` line in `docker-compose.yml` to use the appropriate ID. You can find the list of IDs for the cities starting at line 7 of `conf/cityparams.conf`. If you are using WSL2, you can access the file through the file explorer. Read Windows with WSL2
...
#### Visual Studio Code (HAVE TO USE THIS WITH WSL2)
[Visual Studio Code](https://code.visualstudio.com/download) has a lot of features to run smoothly with Git and integrates well with WSL2
If you are using WSL2 on Windows, follow the next steps! Otherwise you are good to go. Just open the SidewalkWebpage folder!
 
Hit Ctr + Shift + X to open Extension. Then search for and install "Remote - WSL"

Then on the left, there should be a tab with a small computer monitor that says Remote Explorer when you hover over it. Click on it and find your Linux distribution you installed. Then to the right of your Linux distro, there should be a small window that says "Connect to WSL". Click it. 

You are now your distribution. Click the Explorer tab on the top left, click open folder and open the SidewalkWebpage repo.
You are all set up!
  
#### IntelliJ IDEA (CANNOT USE WITH WSL2)
You could also use [IntelliJ IDEA](https://www.jetbrains.com/idea/) IDE for development; however it does not work as well as Visual Studio Code. You should be able to get a [student license](https://www.jetbrains.com/student/) to get the "ultimate" edition of IntelliJ IDEA.
