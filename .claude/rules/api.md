...
- **One name per field, across every format.** A CSV column and its JSON key are the same string, never a re-cased
  or re-worded version of each other. The two single-object endpoints (`overallStats`, `aggregateStats`) get this
  for free: their CSV is `ApiModelUtils.toCsvKeyValueRows(toJson)`, one row per value, keyed by its dotted JSON path
  (`labels.CurbRamp.count`). Rename a field in `toJson` and the CSV follows.
