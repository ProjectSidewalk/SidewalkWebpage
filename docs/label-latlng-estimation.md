...
- `test/service/LatLngEstimationParitySpec.scala` — Scala, 1e-9°; also asserts the fixture's constants equal
  `LatLngEstimation`'s, so a refit fails here until the fixture is regenerated in the same change. This is the suite
  that gates CI (the required "Backend tests" check).
- `test/js/latLngEstimationParity.test.js` — the client formula with the real vendored turf, 1e-8°. The jest suite
  is advisory in CI (`continue-on-error`), so a red run here reports without blocking a merge.
