...
Parallel jobs:
- **evolutions-lint** — host bash; `./db/scripts/lint-evolutions.sh` (also `make lint-evolutions`). Static checks on `conf/evolutions/default/*.sql`: a semicolon mid-`--`-comment (Play splits statements on every `;`, including ones inside comments, then executes the orphaned text — this broke evolution 325, see #4335/#4351) and missing `!Ups`/`!Downs` markers. **Blocking** — fast, deterministic, no DB.
- **evolutions** — applies the whole evolution history **up → down → up** against a real PostGIS DB (`test/evolutions/EvolutionsRoundTripSpec.scala`), into an isolated `evolutions_roundtrip` schema so it never touches the seeded city schemas. Reuses the project `db` image (PostGIS + `sidewalk_login` + the `sidewalk` role, which some evolutions reference by name). Advisory (`continue-on-error`) while we confirm the full 1..N history is clean from scratch (dev is normally dump-seeded, not evolution-built); ramp to blocking once stable. Catches malformed Ups and non-reversible Downs — the highest-blast-radius schema bug class.
