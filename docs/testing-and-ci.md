...
- **Lint gate** (`make lint`: ESLint + Stylelint + HTMLHint + locale key-parity + the `public/css/` layout check, plus the evolutions lint below) was rolled out under **#2487**, sequenced with the in-progress JS ES5→ES2022 migration (dropping linters into CI mid-migration = large, conflict-prone churn). All are **now blocking** steps in the `frontend` job — ESLint (`public/js/` + `public/locales/`), Stylelint (`public/**/*.css`), HTMLHint (`app/views`), `tools/check-locale-parity.mjs`, and `tools/check-css-layout.mjs` (#5030: a page's stylesheet is linked only by that page, page class prefixes stay in the page's own files, every linked stylesheet exists) — each landing once its tree was clean, straight to blocking with no advisory ramp (same as scalafmt/evolutions-lint). **Severity is the gate**: `error` rules block the build (correctness + must-fix smells like `no-unused-vars`/`no-shadow`), while the one `warn` rule on ESLint/Stylelint (`max-len` / `max-line-length`) is deliberately advisory — CLAUDE.md sanctions long-line exceptions, so they're *not* run with `--max-warnings 0`.
