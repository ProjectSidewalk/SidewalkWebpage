...
- **Lint gate** (`make lint`: eslint/htmlhint/stylelint, already configured) is **owned by #2487, not this plan.** That issue sequences the linter rollout with the in-progress JS ES5→ES2022 migration (dropping a linter into CI mid-migration = large, conflict-prone churn). **ESLint (JavaScript) is now wired into CI** — the whole JS tree passed `make eslint` clean as of 2026-07-04, so it went straight to **blocking** (a step in the `frontend` job, `npx eslint public/javascripts/`; no advisory ramp needed, same as scalafmt/evolutions-lint). **Severity is the gate**: `error` rules block the build (correctness + must-fix smells like `no-unused-vars`/`no-shadow`), while the config's one `warn` rule (`max-len`) is deliberately advisory — CLAUDE.md sanctions long-line exceptions, so it's *not* run with `--max-warnings 0`. **htmlhint (HTML) and stylelint (CSS) stay deferred** on the #2487 track until their trees are clean — until then they're a manual pre-check-in expectation (see [`CONTRIBUTING.md`](../CONTRIBUTING.md)), not CI-enforced.
