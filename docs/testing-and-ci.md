...
- **Coverage gate:** the suite measures line + branch coverage (`pytest-cov`) and **fails under 100%** (`--cov-fail-under=100` in `pyproject.toml`). Justified: the scripts are small and now pure, so full correctness coverage is achievable and keeps a new uncovered branch from slipping in. (Contrast the Scala suite, which starts with a low, *ratcheting* scoverage threshold in Phase 4 — a large legacy surface can't jump to 100%.) `main`'s I/O is covered by mocking the network wrappers + `tmp_path`; the only exclusions are the `__main__` guards and one provably-unreachable loop branch (`# pragma: no branch`). Scoping is a bare `--cov` plus `include = ["scripts/*"]`, not `--cov=scripts`: `include` filters what was actually measured, whereas `source` would additionally report the half the running interpreter cannot import as 0% and fail the gate on both. So the gate is on for every invocation, with nothing to remember to pass.
