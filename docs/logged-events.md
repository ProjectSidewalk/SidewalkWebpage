...
| `SubmitFailed` / `SubmitFailedGaveUp` (Validate) | Not user actions — a data POST to `/validationTask` failed and is being retried (`SubmitFailed`, with `attempt` and `error`) or was abandoned (`SubmitFailedGaveUp`), either after the retry cap or immediately on a 4xx, which can't succeed on a resend (#4377). Surfaces flaky-network submission trouble, esp. on mobile (#2745). |
