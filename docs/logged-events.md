...
| `Click_MinimapForwardCrumb` | Click on one of the clickable forward crumbs on the minimap (the amber "next stops" ahead on the current street, #4669/#4655; `panoId` and `rank` notes, rank 1 = nearest). A real forward move that advances the task, unlike a breadcrumb's peek back. Crumb positions come live from the pano viewer's metadata lookup, so they exist on every provider with a location search (GSV, Mapillary, Panoramax, Infra3d), never on Pannellum. |
