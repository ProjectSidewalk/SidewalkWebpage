...
| `MinimapOverview_IntroShown` / `MinimapOverview_End` / `Click_MinimapFitRoute` | The minimap's fitted whole-route overview (#4639): shown automatically at mission start, ended (the `trigger` note says how — `pano-interaction`, `pano-changed`, `zoom`, or `fit-button`), and the manual ⛶ toggle (`mode` note = the resulting state). |
