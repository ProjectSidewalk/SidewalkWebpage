...
| `MinimapOverview_IntroShown` / `MinimapOverview_End` / `Click_MinimapFitRoute` | The minimap's fitted whole-route overview (#4639): shown automatically at mission start, ended (the `trigger` note says how — `pano-interaction`, `pano-changed`, `zoom`, `fit-button`, or `route-inset`), and the manual toggle from the ⛶ button or the route-overview inset (`mode` note = the resulting state, `trigger` note = which control). |
