...
| `Visit_AdminUserDashboard_User=<username>` / `Visit_AdminUser_User=<username>` | Server-logged under the **admin's** user id when they open a user's dashboard as an admin (`/admin/user/:username`) or its Manage user page (`/admin/user/:username/admin`, #4964); the suffix is the viewed user. On the dashboard, the user-facing `Click_module=…` events (routes, re-audit list, map sidebar, …) keep their names and likewise log under the admin, so segment by the visit event to separate an admin's clicks from the user's own. |
