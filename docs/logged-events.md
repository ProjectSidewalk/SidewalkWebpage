...
| `Click_module=StorySectionExpand_labelId=<id>` / `Click_module=StoryComposerOpen_labelId=<id>` | Lived-experience stories on the label-detail card (#4054): expanding the stories disclosure, and opening the composer dialog. Client-logged from `StorySection.js`/`StoryComposer.js`. |
