...
| `RouteBuilder_Click=SavedRoute_Explore_RouteId=<id>` / `SavedRoute_Copy_RouteId=<id>` | Actions on a card in the intro panel's "Your saved routes" section: opening the route in Explore, or copying its share link. Signed-in users see their account's routes there; guests see the device-local list. |
