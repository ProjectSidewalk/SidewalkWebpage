...
| `Click_module=StorySectionExpand_labelId=<id>` / `Click_module=StoryComposerOpen_labelId=<id>` / `Click_module=StorySignInCta_labelId=<id>` / `Click_module=StoryPhotoEnlarge_storyMediaId=<id>` | Lived-experience stories on the label-detail card (#4054): expanding the stories disclosure, opening the composer dialog, clicking the composer's sign-in CTA (stashes the in-progress draft, then bounces to `/signIn`), and enlarging a story photo into the lightbox. Client-logged from `StorySection.js`/`StoryComposer.js`. |
