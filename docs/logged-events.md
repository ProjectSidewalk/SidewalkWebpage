...
| `Click_module=ClearSearchResult` / `KeyboardShortcut_module=ClearSearchResult` | LabelMap and AccessScore: the searched place was taken back off the map (#5321) — by the "Clear search result" button under the search box or the box's own ✕ (`Click`), or by Escape (`KeyboardShortcut`). Logged only when a pin was actually showing, so an Escape with nothing selected leaves no row; the map is not moved back, so a clear says "put the pin away", not "undo the trip". |
