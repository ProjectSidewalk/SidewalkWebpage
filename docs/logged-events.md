...
| `RouteResumeToast_Shown` / `Click_ExitRoute` | A bare `/explore` visit silently resumed a custom-route walk, so the toast saying so was shown (#4833). `Click_ExitRoute` is the user leaving the route for normal exploration — the walk is paused server-side, not discarded, so an explicit re-entry resumes it; the `source` note says which control was used (`toast` or `sidebar`). |
