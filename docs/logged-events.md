...
| `Click_module=Partner_source=<slug>` | Client-logged (`homepage.js`) click on a logo or credit-line link in the landing page's Community Partners section (#4516); the suffix is the partner's slugged name (e.g. `makeability-lab`), read off the anchor's `data-partner-source`; the static created-by strip's anchors carry fixed values (`makeability-lab`, `university-of-washington`, `uic-idhd`). Replaces the pre-#4516 `Click_module=Collaborator_type=…_source=…` event, whose source values were the hardcoded anchor ids. |
