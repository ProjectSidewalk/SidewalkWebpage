...
| `RouteBuilder_Click=SavedRoute_View_RouteId=<id>` / `SavedRoute_Explore_RouteId=<id>` / `SavedRoute_Copy_RouteId=<id>` / `ExitRoutePreview` | Actions on a card in the intro panel's "Your saved routes" section: previewing the route on the map (clicking the card body; `ExitRoutePreview` is the map click that leaves the preview), opening it in Explore, or copying its share link. Signed-in users see their account's routes there; guests see the device-local list. |
