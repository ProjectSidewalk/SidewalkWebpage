...
| `RouteUnavailableToast_Shown` | The visit carried a `?routeId=` naming no live route — mistyped, or a route deleted since the link was shared — so the request was dropped and the toast saying so was shown (#5156). The session otherwise runs as if no route had been asked for, including resuming a walk already in progress, so this fires alongside neither resume toast. A run of them on one route id marks a share link that has gone stale. |
