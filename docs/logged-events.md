...
| `RouteBuilder_Click=RouteMenu_Open` / `ReverseRoute_Popover` / `DeleteRoute_Popover` | The on-route action menu: clicking the drawn route opens it; the two suffixed events are its Reverse-direction and Delete-route buttons (Delete opens the discard-confirm dialog, so a completed delete also emits `ConfirmCancelRoute`). |
...
| `RouteBuilder_Click=CancelRoute` / `ResumeRoute` / `ConfirmCancelRoute` / `BuildNewRoute` | The clear-route flow: opening the discard-confirm dialog, backing out of it, confirming the discard, or starting a fresh route after saving. |
