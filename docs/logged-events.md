...
| `SubmitFailed` / `SubmitFailedGaveUp` (Validate) | Not user actions. `SubmitFailed` is one per failed data POST to `/validationTask`, carrying `attempt`, `status` (absent for a network error) and `error` — it counts failed attempts, not retries, so the last one before a `SubmitFailedGaveUp` was not retried. `SubmitFailedGaveUp` marks abandonment, with `retryable` separating the retry cap from a 4xx given up on immediately because a resend can't succeed (#4377). Surfaces flaky-network submission trouble, esp. on mobile (#2745). |
...
| `PanoViewerCreateFailed` (Explore) | Not a user action — the pano viewer wouldn't initialize on the assigned street for a reason that says nothing about the street's imagery (the maps library failing to load, a provider error, a network blip). Explore stops there and shows the user a retry message, and crucially writes **nothing** to the database: before #4918 this path reported the street as imagery-less and reloaded, which marked it audited and took it out of the assignment rotation. Read a burst as provider or network trouble, not as streets without coverage. |
...
| `NoImageryFlagLimitReached` (Explore) | Not a user action — a session hit the cap on consecutive streets automatically flagged as imagery-less (`NoImageryFlagGuard.MAX_CONSECUTIVE_FLAGS`, #4918) and stopped flagging. Fires from both flagging paths, the page-load one in `PanoManager` and the mid-street one in `NavigationService`. Nothing here is user-initiated. It means the client *declined* to mark further streets audited, so it is the signal that the guard did its job; the streets after it are untouched rather than lost. One per stuck session, so volume counts broken sessions, not streets. |
