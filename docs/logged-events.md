...
| `Click_module=StorySectionExpand_labelId=<id>` / `Click_module=StoryComposerOpen_labelId=<id>` / `Click_module=StoryPhotoEnlarge_storyMediaId=<id>` | Lived-experience stories on the label-detail card (#4054): expanding the stories disclosure, opening the composer dialog, and enlarging a story photo into the lightbox. Client-logged from `StorySection.js`/`StoryComposer.js`. |
