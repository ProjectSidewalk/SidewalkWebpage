...
| `ImagerySkipBudgetSpent` (Explore) | Not a user action — a session hit the cap on consecutive streets auto-skipped for missing imagery (`ImagerySkipGuard.MAX_CONSECUTIVE_SKIPS`, #4918) and stopped skipping. Fires from both skip paths, the page-load one in `PanoManager` and the mid-street one in `NavigationService`. It means the client *declined* to mark further streets audited, so it is the signal that the guard did its job; the streets after it are untouched rather than lost. One per stuck session, so volume counts broken sessions, not streets. |
