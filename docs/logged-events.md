...
a discrete action (e.g. `Click_module=SaveSettings` — which the `/welcome` privacy toggles also emit, see `Click_module=WelcomePrivacy_*` below —, `Click_module=CreateTeam`, `Click_module=LeaveTeam`, `Click_module=MistakeVote_agrees=<bool>`, `Click_module=MistakeNote`).
...
| `Visit_Audit_UnresolvableRouteId=<id>` / `RouteUnavailableToast_Shown` | The visit carried a `?routeId=` naming no live route — mistyped, or a route deleted since the link was shared — so the request was dropped (#5156). The session otherwise runs as if no route had been asked for, including resuming a walk already in progress, so the visit *also* logs its usual `Visit_Audit*` line and fires neither resume toast. The id that failed is only on the server-side line, which is what tells a stale share link (many users, one id) from a typo (one user, one-off); the client-side line marks that the user was actually told, which a visit landing in the tutorial is not — the notice waits for the post-tutorial load. |
...
| `ValidateInputDropped_Loading` (Validate) | Not a user action in the usual sense — the validator did act, and the tool refused it because the next label's pano hadn't arrived yet (#5211). In that window `getCurrentLabel()` is already the next label while the pano on screen is still the previous one, so a verdict taken then would be stored against imagery nobody saw. The `source` note says what was dropped, and how far it got: a verdict button (`Agree` / `Disagree` / `Unsure`), a reason option for one of those verdicts (`DisagreeReason` / `UnsureReason`), a mobile skip-reason button (`DisagreeReason_Skip` / `UnsureReason_Skip`), an Expert Validate severity or tag edit (`Severity` / `TagAdd` / `TagRemove`), a `Submit=<verdict>` from a menu, a `Validate=<verdict>` that reached the container itself, an `Undo`, or a `NextLabel` advance. Rare by design, since the UI is dimmed and pointer-blocked for the same window; what it catches is what CSS can't — a keypress on a button that kept focus, or a tap that beat the class onto the page. Read the sources in two groups. The ones that reach the label without submitting — `DisagreeReason`, `UnsureReason`, `Severity`, `TagAdd`, `TagRemove` — are the native-activation cases: those controls hold focus after a click, and once the tool stops intercepting keys the browser activates them on its own (Enter on a button, an arrow key roving the severity radios, whose keydown the pano viewers stop propagating but never preventDefault). They track a double-Enter or arrow-key habit rather than slow loads. The rest are real submits and advances, and a rise in *those* means loads are getting slow enough for validators to run into them, so read them against `Viewer_Pannellum`. |
| `ValidateRenderFailed` (Validate) | Not a user action — loading the next label threw somewhere after the pano request, so the render never finished (#5211). The tool hands itself back rather than staying locked, which means this event is the *only* trace: the two callers either swallow the rejection (`Form`, after a mission-complete submit) or drop it (`moveToNextLabel`), so nothing else reaches the server. The `error` note is the exception message. Before the lock was released on a throw this announced itself as an endless run of `ValidateInputDropped_Loading` from one user, so a *fall* in that pattern paired with these is the fix working, not the failure going away. Any occurrence is a bug — the render path is all local UI work. |
...
| `Click_module=WelcomePrivacy_setting=<Leaderboard\|PublicProfile>_value=<true\|false>` | A privacy toggle on the post-registration `/welcome` page (#4375), client-logged from `common/WelcomePrivacy.js`; `value` is the state the box was moved *to*. Changing the same two flags later from Dashboard → Settings has no per-flag event — that save logs only `Click_module=SaveSettings` — so this event covers the welcome page alone, and the flags' stored values are what say where a user ended up. A failed save puts the box back and the event still stands, so a count here is intent rather than a confirmed write. **These toggles post to the Settings save endpoint, so each one also emits a server-side `Click_module=SaveSettings`:** anyone counting Settings-page saves is counting welcome-page privacy toggles too, and a new user who flips both boxes contributes two. Subtract this event to separate them. |
...
| `Click_module=LabelDetail_action=<PanoInfoButton\|PanoInfoCopyToClipboard\|PanoInfoViewInPano\|ViewOnLabelMap\|ExploreHere\|HideLabel\|ShowLabel>_labelId=<id>` / `Click_module=LabelPopup_action=<NextLabel\|PrevLabel>_labelId=<id>` | The shared Label Detail Card (#4572) on its `webpage_activity`-logged hosts (LabelMap, Gallery, dashboards, admin, share page): opening the Details popover, its copy-to-clipboard and view-in-provider actions, the "View on Label Map" hop, the "Explore the sidewalks here" hop into a free-exploration Explore session seeded at the label's pano + POV (#4637), the Hide/Show-label toggle over the imagery (#2477 — only the click is logged; the state it sets then carries to later labels silently), and the popup's prev/next nearby-label arrows (`labelId` is the shown label; the arrows log the label paged *from*). On Explore/Validate the same popover logs through those tools' trackers as `PanoInfo*_Click` instead. The two paging actions also have a `KeyboardShortcut_module=LabelPopup_action=<NextLabel\|PrevLabel>_labelId=<id>` form, which is what the card's left/right arrow shortcuts and Enter/Space on a focused arrow log as (#5194); the Gallery's expanded view pages through its own tracker instead (`NextLabelClick` / `NextLabelKeyboardShortcut`). |
| `Click_module=LabelDetail_action=ClearVote_result=<Agree\|Disagree\|Unsure>_labelId=<id>` / `KeyboardShortcut_module=LabelDetail_action=ClearVote_result=<…>_labelId=<id>` | The user cleared their validation of a label by choosing again the vote they'd already cast on the label detail card (#4653); `result` is the vote that was cleared, and the `KeyboardShortcut_` form is the A/Y, D/N and U shortcuts plus Enter/Space on a focused vote button (#5194). **Casting** a vote isn't logged here — it lands in `label_validation` with a `source` naming the surface — but clearing one *deletes* that row, so this event is the only record it happened. The Gallery's small cards vote through their own tracker instead, where the same action reads `Validate_MenuClickClear<Result>` / `Validate_ThumbsMenuClickClear<Result>` (or `…KeyboardShortcutClear<Result>`). |
...
| `Click_MinimapForwardCrumb` | Click on a clickable crumb on the minimap (#4669/#4655; `panoId`, `kind` and `rank` notes). `kind` is `route` for a stop ahead on the current street (rank 1 = nearest) or `link` for the destination of one of the current pano's other arrows. A real forward move that advances the task, unlike a breadcrumb's peek back. Crumb positions come live from the pano viewer's metadata lookups, so they exist on every provider with a location search (GSV, Mapillary, Panoramax, Infra3d), never on Pannellum. |
| `ForwardCrumbMove_Fallback` | A forward crumb's pano id no longer loaded (GSV retires panos), so the click fell back to the coordinate search `moveForward()` uses at the crumb's position (`panoId` note). Logged after the `Click_MinimapForwardCrumb` it follows. |
