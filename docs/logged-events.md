...
| `Click_module=SearchPin_lat=<lat>_lng=<lng>` | LabelMap and AccessScore: the searched place's pin was activated, which opens the "Explore the sidewalks here" invitation and puts focus on its button (#5321). Not a navigation: only the button's own `ExploreSidewalksHere` row above means Explore was opened. |
| `Click_module=ClearSearchResult` / `KeyboardShortcut_module=ClearSearchResult` | LabelMap and AccessScore: the searched place was taken back off the map (#5321) — by the search box's own ✕ or AccessScore's "Reset everything" (`Click`), or by Escape (`KeyboardShortcut`). Logged only when a pin was actually showing, so an Escape with nothing selected leaves no row; the map is not moved back, so a clear says "put the pin away", not "undo the trip". |
