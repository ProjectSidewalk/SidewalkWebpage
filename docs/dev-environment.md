...
| `relation "role" does not exist` while a schema is applying evolutions | That schema is behind evolution 371, which dropped the shared `sidewalk_login.role` lookup table. Evolutions 270, 295 and 355 read it, so a schema below any of them can no longer replay them and is stuck. Bring the schema forward from a dump that is already past 355, or re-import it — editing those old evolution files is *not* the fix, since Play would then revert every schema that already applied them. Same cause if a freshly onboarded city dies mid-replay: the committed `db/sidewalk_init-dump` needs regenerating past 371. |
