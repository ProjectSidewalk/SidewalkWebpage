...
2. **Google Analytics** — with `ga-service-account.json` in the repo root (one-time setup in
   ...
   standalone later. Then, when `gcloud` is signed in as the owner of the production GCP project, adds both URLs'
   hostnames to the Maps key's referrers (`tools/maps_key_referrers.py`, which only ever appends); without them the
   city's map and panos don't load. Skipped with a pointer otherwise.
...
- **Server.** `scp db/<schema>-dump <netid>@makelab1.cs.washington.edu:/www/sidewalk/new-city-dumps/<schema>-empty-dump`
  ...
  skipped them (`python3 tools/maps_key_referrers.py <city-id>`), DNS, and the PR with the config, message, and docs changes. Where the tooling can't be
