...
-- Add a new tag for pedestrian signals in Zurich.
INSERT INTO tag (tag_id, label_type_id, tag, mutually_exclusive_with)
SELECT 86, label_type_id, 'yellow box, accessibility features not visible', NULL
FROM label_type
WHERE label_type.label_type = 'Signal';
...
SET excluded_tags = excluded_tags || '[
  {
    "label_type": "Signal",
    "tag": "yellow box, accessibility features not visible"
  }
]'::jsonb
...
UPDATE label
SET tags = ARRAY(SELECT elem FROM UNNEST(tags) AS elem WHERE elem NOT IN ('yellow box, accessibility features not visible'))
FROM label_type
WHERE label.label_type_id = label_type.label_type_id
    AND label_type.label_type = 'Signal';

UPDATE label_history
SET tags = ARRAY(SELECT elem FROM UNNEST(label_history.tags) AS elem WHERE elem NOT IN ('yellow box, accessibility features not visible'))
FROM label
...
WHERE tag.label_type_id = label_type.label_type_id
    AND label_type.label_type = 'Signal' AND tag.tag = 'yellow box, accessibility features not visible';
