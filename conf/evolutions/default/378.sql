...
-- card's Delete control (#5015) removes it outright. webpage_activity keeps only the fact of the change -- who acted,
-- when, on which label -- so a validator's own words are the part that cannot be reconstructed.
--
-- A history table rather than a `deleted` flag on validation_task_comment. A flag has nothing to say about text
-- replaced in place, which is the highest-volume loss of the three, and a soft-deleted row would still occupy the
-- (label_id, user_id) pair 359.sql made UNIQUE, forcing that constraint into a partial index three evolutions after
-- it was added. label_history is the precedent: a user-authored value whose earlier states are research data.
--
-- Append-only. Even an explicit Delete removes the comment from every read path in the tool and adds a row here
-- rather than erasing it. If a contributor ever asks for their words to be truly gone, that is a hand-run cleanup.

-- edit: superseded by new text from the same user, whether typed on the label card or carried in with a new vote.
...
-- Mirrors the columns of the validation_task_comment row it supersedes, pano and pose included, so a version can
-- still be shown where it was written.
...
-- Every read is "the history of this user's comment on this label" -- the pair validation_task_comment is keyed by,
-- so the live row joins on it too. Ordering within a pair is left to a sort of the handful of rows one pair has.
