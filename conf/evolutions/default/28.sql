...
UPDATE label
  SET street_edge_id = (
      ...
      ORDER BY ST_Distance(s.geom,ST_SetSRID(ST_MakePoint((
                                                SELECT lp.lng FROM label_point AS lp
                                                      WHERE lp.label_id = label.label_id
                                                ), (
                                                SELECT lp.lat FROM label_point AS lp
                                                      WHERE lp.label_id = label.label_id
                                                )),Find_SRID('sidewalk', 'street_edge', 'geom'))) ASC
      LIMIT 1
  )
  WHERE (SELECT lp.lat FROM label_point AS lp WHERE lp.label_id = label.label_id LIMIT 1) IS NOT NULL AND
        (SELECT lp.lng FROM label_point AS lp WHERE lp.label_id = label.label_id) IS NOT NULL;

UPDATE label
  SET street_edge_id = -1
