...
CREATE TEMP TABLE stale_votes_390 AS
...
WHERE label_validation_id IN (SELECT label_validation_id FROM stale_votes_390);

DELETE FROM label_history
USING label_edit
WHERE label_edit.label_edit_id = label_history.label_edit_id
  AND label_edit.label_validation_id IN (SELECT label_validation_id FROM stale_votes_390);

DELETE FROM label_edit WHERE label_validation_id IN (SELECT label_validation_id FROM stale_votes_390);

DELETE FROM label_validation WHERE label_validation_id IN (SELECT label_validation_id FROM stale_votes_390);

DROP TABLE stale_votes_390;
