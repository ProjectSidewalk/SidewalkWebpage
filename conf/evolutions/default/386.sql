...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes
    ...
    -- sign-up's write order, lets an in-flight sign-up finish rather than deadlock against the ALTERs below. EXCLUSIVE
    -- rather than ACCESS EXCLUSIVE, so page loads in every city (which read sidewalk_user) keep working through the
    -- cleanup, and only each ALTER at the end blocks readers, for the seconds its index build takes.
    LOCK TABLE sidewalk_login.sidewalk_user, sidewalk_login.login_info, sidewalk_login.user_login_info,
      sidewalk_login.user_password_info, sidewalk_login.user_role IN EXCLUSIVE MODE;;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes
      ...
      -- Login rows no account points at (1,187 on prod, from the pre-v9 sign-up code's non-atomic writes). None of them
      -- carries both a live account's email and a password (checked on prod), so no sign-in depends on one.
      ...
      -- Several passwords for one login row, or several role rows for one account: keep the newest. None on prod.
      DELETE FROM sidewalk_login.user_password_info
      ...
      USING (SELECT user_id, max(user_role_id) AS keep_id
             FROM sidewalk_login.user_role GROUP BY user_id HAVING count(*) > 1) AS multi
      WHERE user_role.user_id = multi.user_id AND user_role.user_role_id <> multi.keep_id;;
      ...
      -- Carries the renames above over to the login rows (and fixes any other drift, none on prod). A renamed email can
      -- outgrow varchar(254), so the key becomes text like sidewalk_user.email (a catalog change, no rewrite).
      ALTER TABLE sidewalk_login.login_info ALTER COLUMN provider_key TYPE text;;
...
ALTER TABLE sidewalk_login.login_info
  ...
  ALTER COLUMN provider_key TYPE VARCHAR(254);
ALTER TABLE sidewalk_login.sidewalk_user
