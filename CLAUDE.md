...
Three standalone scripts in **`scripts/`**, invoked out-of-band rather than from the running web app. Full usage in [`scripts/README.md`](scripts/README.md):
...
- `scripts/onboard_city.py` — builds a new city's `qgis_road`/`qgis_region` staging data from open sources (#4291), consumed by `db/scripts/fill-new-schema.sh`. Never writes to the database — it emits a QA GeoPackage, a SQL load file, and a report under `db/onboarding/<city-id>/` (git-ignored). Run it via `make build-city-data id=<city-id> args="..."`; once the GeoPackage passes QA, `make onboard-city id=<city-id>` (`tools/setup_new_city.py`, host-side) chains the whole remaining setup — config registration, GA properties (`tools/create_ga_properties.py`; its `ga-service-account.json` key is git-ignored and deny-listed from Claude like `docker-compose.override.yml`), schema creation, evolutions, the fill, and the imagery scan. See `db/scripts/README.md` → "Standing up a brand-new city".
...
Each script's pure logic is refactored into importable functions and **unit-tested** under `test/python/` (`pytest`). Keep I/O (HTTP/file) in thin wrappers and `main` so the logic stays testable. The suite splits along the same line: `make test-python` runs both halves, each taking the whole directory *minus* the files the other interpreter owns (`--ignore`, in the `Makefile`'s `pytest-args-*` and mirrored in the CI matrix), so **a new test file runs in both halves by default**. Coverage is always on and gated at 100% via `source = ["scripts"]`, so a script arriving with no tests is reported at 0%; each half sets **`COVERAGE_OMIT`** (and, on the 3.8 half, **`COVERAGE_OMIT2`**) to the scripts it can't import.
