...
**The web image carries two interpreters, and which one a script uses is a real constraint, not a preference (#4396).** `python3` is the base image's **3.8** — EOL, but it is what the deployed app shells out to, so `label_clustering.py` and its `requirements.txt` pins must stay installable there. `python3.13` (a uv-fetched standalone CPython) is for everything offline, and is where `requirements-offline-tools.txt` — `check_streets_for_imagery.py`'s stack, which needs ≥ 3.11 — is installed. This mirrors prod (makelab1: the app runs on Rocky's system Python, user accounts have 3.13). So: **run offline tooling as `python3.13 scripts/...`, and don't add new libraries to `requirements.txt`** — anything current has dropped 3.8. `requirements-dev.txt` (pytest) installs into both and uses environment markers to give each the newest release it can take.

Each script's pure logic is refactored into importable functions and **unit-tested** under `test/python/` (`pytest`). Keep I/O (HTTP/file) in thin wrappers and `main` so the logic stays testable. The suite splits along the same line — `make test-python` runs `test-python-app` (label_clustering under 3.8) then `test-python-tools` (the offline tooling under 3.13). Each half runs the whole directory *minus* the one file the other interpreter owns (`--ignore`, listed once in the `Makefile`'s `pytest-args-*` and mirrored in the CI matrix), so **a new test file runs in both halves by default** — if it can only work on one, add an `--ignore` for it to the other. Coverage is enabled in `pyproject.toml` with a bare `--cov` plus `source = ["scripts"]`, so the 100% gate is on for every invocation and a script that arrives with no tests at all is reported at 0% rather than going unmeasured. Each half sets **`COVERAGE_OMIT`** to the one script its interpreter can't import (the `cov-omit-*` variables in the `Makefile`, `coverage-omit` in the CI matrix); with it unset, nothing is omitted and a hand-run fails the gate loudly instead of quietly measuring less than it appears to.
