...
Each script's pure logic is refactored into importable functions and **unit-tested** under `test/python/` (`pytest`). Keep I/O (HTTP/file) in thin wrappers and `main` so the logic stays testable. The suite splits along the same line — `make test-python` runs `test-python-app` (label_clustering under 3.8) then `test-python-tools` (the offline tooling under 3.13). Each half runs the whole directory *minus* the one file the other interpreter owns (`--ignore`, listed once in the `Makefile`'s `pytest-args-*` and mirrored in the CI matrix), so **a new test file runs in both halves by default** — if it can only work on one, add an `--ignore` for it to the other. Coverage is enabled in `pyproject.toml` with a bare `--cov` plus `include = ["scripts/*"]` rather than `--cov=scripts`: `include` filters what was measured instead of enumerating what should have been, so each half reports exactly the script it imported and no invocation can lose the 100% gate.
