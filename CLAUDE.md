...
**Reference every asset through `assets.path("...")` in Twirl, never a hardcoded `/assets/...` string.** `stage`/`dist` content-fingerprints assets (sbt-digest), and only `assets.path` resolves to the fingerprinted URL that gets a one-year `immutable` cache; a hardcoded path falls back to the one-hour default, so a returning visitor pays a conditional GET per asset per hour, and a deploy that swaps a file's bytes reaches a cached client only once that hour is up — an interval where fresh HTML (never cached) points at stale bytes. Full contract: [`docs/deployment-and-stages.md`](docs/deployment-and-stages.md) → "Asset caching".

**In JavaScript the equivalent is `util.assetPath("images/icons/openhand.cur")`** (`public/js/common/utilities.js`, loaded on every page) — **never a hardcoded `/assets/...` string** (#4893). A `.js` file can't call `assets.path`, so the app publishes the answers: `build.sbt` generates `models.utils.AssetInventory` from `assetManifestPrefixes` (the asset families JS draws from), `AssetManifestService` resolves each through `AssetsFinder` at startup, and `main.scala.html` stamps the `{logical path → md5}` map as `window.assetDigests` **ahead of `utilities.js`** — the tool bundles resolve icon URLs in module-level constants at script-eval time, so a later stamp would be read as `undefined` on first load. Take the argument's logical path from `public/` with no leading slash and no `assets/` prefix; build the whole path first for a dynamic one (`` util.assetPath(`images/badges/badge_labels_badge${level}.png`) ``) rather than concatenating onto a resolved URL. A base *directory* can never be fingerprinted (sbt-digest prefixes the filename), so don't reintroduce one. Under dev `sbt run` the stamp is `{}` and every lookup falls back to the plain `/assets/<path>`. `make lint-asset-paths` is the gate; the one allowlisted exception is `AppManager.js`'s i18next `loadPath` template.
...
- After editing frontend files, lint what you touched and get to zero before the change is done. Every frontend linter is a **blocking CI gate** (steps in the `frontend` job — see Continuous integration), the JS/CSS/HTML/i18n counterpart to the scalafmt rule above, so a finding fails the build. The whole tree is lint-clean (#2487), so any finding is from your change.
...
## Linting Rules (every frontend linter must pass before check-in — blocking CI gates, like Scala `scalafmt`; see Continuous integration)
...
```bash
make lint           # eslint + stylelint + htmlhint + lint-locales + lint-css-layout + lint-asset-paths + lint-evolutions (all of it)
...
make lint-asset-paths # public/js/ asset URLs: no hardcoded /assets/, util.assetPath args are real (tools/check-asset-paths.mjs)
...
**Every frontend linter must pass (zero errors) before code is checked in** — like scalafmt for Scala. The trees are
fully lint-clean (#2487), so a bare run should come back green and any finding is yours: `make lint-fix` handles the
mechanical ESLint + Stylelint fixes, hand-fix the rest, then confirm with `make lint`. All of them are **blocking CI gates**
...
Config: `eslint.config.js`, `stylelint.config.mjs`, `.htmlhintrc`; cross-locale parity is `tools/check-locale-parity.mjs`, the CSS layout `tools/check-css-layout.mjs`, and the asset paths `tools/check-asset-paths.mjs`. Scala formatting is `.scalafmt.conf`.
