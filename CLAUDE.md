...
- `scripts/onboard_city.py` — builds a new city's `qgis_road`/`qgis_region` staging data from open sources (#4291), consumed by `db/scripts/fill-new-schema.sh`. Never writes to the database — it emits a QA GeoPackage, a SQL load file, and a report under `db/onboarding/<city-id>/` (git-ignored).
