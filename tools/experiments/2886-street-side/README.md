...
wants next · Seattle, every real label · tool and every number in `tools/experiments/2886-street-side/`
...
> Reproduce (offline, from the dev database; ~6 min): `pipeline.md`. The SDOT layers were pulled
...
Everything is offline and lives in `tools/experiments/2886-street-side/` (download, load, compute, export, analyze; `pipeline.md` there).
...
![Labels by distance to the centerline, and agreement by band](fig_agreement_by_distance.png)
...
![Geometric side flipped by the reposition, by current distance band](fig_stability.png)
...
![Accuracy by distance to the centerline, three truth sets](fig_truth_by_distance.png)
...
![Accuracy by camera offset from the audited centerline](fig_truth_by_offset.png)
...
![Accuracy by heading margin, three truth sets](fig_truth_by_axis.png)
...
![Geometric accuracy by camera-to-label distance, and by distance to the centerline in 0.5 m bands](fig_calibration.png)
...
![Accuracy vs coverage as each method abstains below its own margin](fig_abstention.png)
...
![Map and street-level image of label 267987: camera 1 m off the street and 5 m from the label; both readings point right, and SDOT's ramp point sits beside the label](fig_case_agree.png)
...
![Map and street-level image of label 114352: the camera sits 12 m off the audited street on the cross street; its ray crosses the audited centerline, so the heading method says right while the label and the SDOT ramp are on the left](fig_case_corner.png)
...
![Map and street-level image of label 293679: 0.3 m from the centerline, seen from 15 m away on the cross street; the hollow marker shows where it sat before the #4818 reposition moved it across the line](fig_case_near_line.png)
...
![Map and street-level image of label 99610: 31 m from the camera and 10 m from the centerline; the camera ray runs along the street and both readings say right](fig_case_far.png)
...
![Map and street-level image of label 299692: on the unimproved side of a one-sided street, 4 m from the centerline; both readings agree with SDOT](fig_case_ns_bare.png)
...
![Map and street-level image of label 285511: the label sits on the walkway SDOT records as paved; both readings put it there](fig_case_ns_paved.png)
...
(`hand-label-sample.csv`, stratified by type and by whether the methods agree, with a Gallery
...
```bash
python3.13 tools/experiments/2886-street-side/download_sdot.py tools/experiments/2886-street-side/data           # SDOT layers (network)
docker exec -w /home/<worktree> projectsidewalk-web python3.13 tools/experiments/2886-street-side/street_side.py load
docker exec -w /home/<worktree> projectsidewalk-web python3.13 tools/experiments/2886-street-side/street_side.py compute   # ~4 min
docker exec -w /home/<worktree> projectsidewalk-web python3.13 tools/experiments/2886-street-side/street_side.py export
docker exec -w /home/<worktree> projectsidewalk-web python3.13 tools/experiments/2886-street-side/analyze_street_side.py
python3 tools/experiments/2886-street-side/fetch_share_images.py                                                # §4.7 picks + images (network)
docker exec -w /home/<worktree> projectsidewalk-web python3.13 tools/experiments/2886-street-side/case_maps.py      # §4.7 figures
```

`tools/experiments/2886-street-side/out/summary.json` holds every number quoted above; `tables.md` every table, including the ones
