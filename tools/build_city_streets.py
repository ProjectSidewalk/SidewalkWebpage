...
def wkt(geom) -> str:
    """WKT with coordinates rounded to 7 decimals (~1 cm), which keeps the SQL file small."""
...
def build(args: argparse.Namespace) -> int:
    ...
    if not re.fullmatch(r"[a-z_][a-z0-9_]*", name_col):
        raise SystemExit(f"--region-name-prop {name_col!r} must be a plain lowercase SQL identifier")
    lines_out = [
