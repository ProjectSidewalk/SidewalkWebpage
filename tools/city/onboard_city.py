...
Workflow:
  ...
  3. Run ``make onboard-city id=<city-id>`` (tools/city/setup_new_city.py), which chains the rest of the setup — configs,
...
def find_overlapping_streets(roads, tol_m=0.5, min_len_m=10.0, min_coverage=0.8):
    """
    Finds pairs of streets drawn on top of each other (#3067), by the rule tools/one-off/3067-find-duplicate-streets.sql
...
def write_sql(path, roads, regions):
    ...
    """
    lines = [
        '-- Generated by tools/city/onboard_city.py — staging tables consumed by db/scripts/fill-new-schema.sh.',
...
def write_report(path, args, region_source, roads, regions, dropped, stats, coverage, heal_stats,
    ...
    lines = [
        ...
        '',
        f'- Generated: {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")} by `tools/city/onboard_city.py`',
