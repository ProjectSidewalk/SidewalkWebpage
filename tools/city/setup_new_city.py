...
Run after tools/city/onboard_city.py has produced db/onboarding/<city-id>/ and the GeoPackage has been QA'd:

    make onboard-city id=laurens-ia        (host-side; wraps `python3 tools/city/setup_new_city.py laurens-ia`)
...
It chains every remaining setup step, pausing only where a human is required:
  ...
  2. Creates the city's GA4 properties and fills the measurement + property ids (tools/city/create_ga_properties.py) —
     ...
     (tools/city/maps_key_referrers.py); when gcloud can't read or edit the key, skipped with a pointer.
  ...
  7. Runs the tools/city/check_streets_for_imagery.py scan for the city's imagery provider in the web container (which
  ...
  8. Samples the street gradient (#5223, docs/street-gradient.md): exports the streets with the build's
     ...
     runs tools/city/street_gradient.py in the web container, and imports the result into street_gradient. A country
...
#                     probe of a published port reads "taken" whatever is running (measured). tools/dev/qa-worktree.sh
#                     asks the same way.
...
def schema_name(city_id):
    """Same derivation as tools/city/onboard_city.py: full city id, hyphens as underscores."""
    return 'sidewalk_' + city_id.replace('-', '_')


def valid_city_id(value):
    """
    argparse type for the city id — the same rule tools/city/onboard_city.py applies to --city-id (kept local: this
...
Server handoff for {city_id}:
  ...
  3. DNS, and confirm {test_url} and {prod_url} are on the Maps API key's referrers, or the city's map and panos
     won't load: step 2 offers to add them, and `python3 tools/city/maps_key_referrers.py {city_id}`
...
def run_imagery_scan(schema, city_id, pano_type):
    ...
    subprocess.run(docker_argv(WEB_CONTAINER, 'python3.13', 'tools/city/check_streets_for_imagery.py',
...
def run_street_gradient(schema, city_id, sampler_args=()):
    ...
    Args:
        sampler_args: Extra flags for tools/city/street_gradient.py: --dem-dir, --dem-name and --dem-resolution-m for
    ...
    print('  Sampling the elevation model along every street (seconds for most cities, a minute or two for a '
          'large one)...')
    sampler = subprocess.Popen(docker_argv(WEB_CONTAINER, 'python3.13', 'tools/city/street_gradient.py',
  ...
  or sample and import by hand:
    ...
    make import-street-gradient args="{schema} onboarding/{city_id}/street_gradient.csv"''')
            return 'no_source'
        sys.exit(f'error: tools/city/street_gradient.py failed (exit {status}); its output is above. Fix the cause '
...
def offer_maps_key_referrers(city_id):
    ...
    try:
        ...
        if not to_add:
            ...
            print(f'  Not added; run `python3 tools/city/maps_key_referrers.py {city_id}` before launch.')
        ...
        print(f'  Maps key referrers: skipping, since {err}. Run `python3 tools/city/maps_key_referrers.py {city_id}` once '
...
def main(argv=None):
    ...
    parser.add_argument('city_id', type=valid_city_id,
                        help='The cityparams city id, e.g. "laurens-ia" (must match the tools/city/onboard_city.py '
    ...
    parser.add_argument('--dem-dir', help='Step 8, for a country with no registered elevation model: a directory of '
                                          'hand-downloaded rasters, passed to tools/city/street_gradient.py with the '
    ...
    if not create_ga_properties.KEY_FILE.is_file():
        print(f'  No {create_ga_properties.KEY_FILE.name} in the repo root; skipping — see '
              'tools/city/create_ga_properties.py for the one-time setup, then run it standalone.')
