...
replacement: it takes the two CSV exports beside it (pool.sql and validations.sql, run on every city at once by
sidewalk-server-tools/run-query-in-every-city.sh) and answers five questions. The first four run once with NoSidewalk
...
Usage (docs/validation-queue.md has the export commands):

    python3.13 tools/validation_queue/analyze_validation_queue.py \\
        --pool scratchpad/validation-queue-pool.csv \\
        --validations scratchpad/validation-queue-validations.csv \\
...
def load_pool(path, city=None):
    ...
    @param city: Keep only this city's rows of a merged every-city export.
    ...
    with open(path, newline="") as handle:
        ...
        for record in _city_rows(reader, city):
...
def load_validations(path, city=None):
    """Read validations.sql's CSV into the row dicts `replay_margins` expects, in export order.

    @param path: Path to the exported validations.csv.
    @param city: Keep only this city's rows of a merged every-city export.
    @returns: One dict per validation.
    """
    out = []
    with open(path, newline="") as handle:
        for record in _city_rows(csv.DictReader(handle), city):
            out.append(
                {
...
def build_report(pool, validations, schema, seed=4715, votes=20000, missions_per_type=2000):
    ...
    parts = [
        ...
        "",
        "Generated by `tools/validation_queue/analyze_validation_queue.py` from `pool.sql` and `validations.sql` "
        "beside it. Every number below comes from that run; none is typed by hand.",
        "",
...
def main(argv=None):
    ...
    parser.add_argument("--pool", required=True, help="CSV that pool.sql (beside this script) produced")
    parser.add_argument("--validations", required=True, help="CSV that validations.sql produced")
    parser.add_argument("--city", help="which city's rows to analyze, when the CSVs hold every city (the runner's -m)")
    parser.add_argument("--schema", help="label for the report header; defaults to --city")
    ...
    schema = args.schema or args.city or "unknown"
    report = build_report(load_pool(args.pool, args.city), load_validations(args.validations, args.city), schema,
                          seed=args.seed, votes=args.votes, missions_per_type=args.missions)
    if args.out:
