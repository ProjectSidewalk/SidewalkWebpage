...
This is a fast, deterministic, DB-free static check (like db/scripts/lint-evolutions.sh and tools/lint/check-locale-parity.mjs)
...
def main():
    routes_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent.parent / "conf" / "routes"
