...
class MissionServiceImpl @Inject() (
   ...
   */
  private def updateCompleteAndGetNextMission(
      userId: String,
      regionId: Int,
      missionId: Int,
      skipped: Boolean
    ...
    queryMissionTableExploreMissions(
      actions, userId, Some(regionId), Some(false), Some(missionId), None, None, Some(skipped)
    )
  }
   ...
   */
  def updateCompleteAndGetNextMission(
      userId: String,
      regionId: Int,
      missionId: Int,
      distanceProgress: Float,
    ...
    queryMissionTableExploreMissions(
      actions, userId, Some(regionId), Some(false), Some(missionId), Some(distanceProgress), auditTaskId, Some(skipped)
    )
  }
  ...
  def updateExploreProgressOnly(
    ...
    queryMissionTableExploreMissions(
      Seq("updateProgress"), userId, None, None, Some(missionId), Some(distanceProgress), auditTaskId, None
    )
  }
        ...
        missionTable
          ...
          .flatMap { completedOnboarding =>
            ...
            if (!completedOnboarding || retakingTutorial.get) {
              ...
              missionTable
                .getIncompleteAuditOnboardingMission(userId)
                .flatMap {
              ...
              missionTable
                .getCurrentMissionInRegion(userId, regionId.get)
                .flatMap {
