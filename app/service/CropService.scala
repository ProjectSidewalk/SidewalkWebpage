...
trait CropService {
  ...
  /**
   * Records the provenance of every crop on disk that has none, and cuts a crop for every live label that has none
   * and whose pano has a self-hosted image. At most one run at a time: a second call while one is in flight fails
...
class CropServiceImpl @Inject() (
  ...
  private class Counts {
    var panosOpened, panosWithoutBackup, cropsWritten, shiftedVertically, outOfFrame, dimsMismatch, dimsUnverified,
        provenanceExplore, provenanceWindow, provenanceUnresolved, errors = 0
    ...
    def result: CropRunResult = CropRunResult(
      ...
      provenanceExplore, provenanceWindow, provenanceUnresolved, errors
    )
  }
