    ...
    def getExploreDataAction = for {
      ...
      task: Option[NewTask] <- {
        if (mission.missionType == MissionType.AuditOnboarding) {
          ...
          // AuditTaskTable.resumableTasksForUser applies this same test to the mid-session next-street pick (#5370),
          // so the two definitions of "resumable" have to move together.
