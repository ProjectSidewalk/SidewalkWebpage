...
object PersistentMediaDirCheck {
  ...
  val persistentDirs: Seq[PersistentDir] = Seq(
    // Crops and share previews are derived: a crop is re-cut from pano imagery by the nightly crop job and a share
    // preview rebuilds on demand, so losing them costs rebuild time, not content.
