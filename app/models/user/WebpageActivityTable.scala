...
case class WebpageActivity(webpageActivityId: Int, userId: String, description: String, timestamp: java.sql.Timestamp)
...
class WebpageActivityTable(tag: Tag) extends Table[WebpageActivity](tag, Some("sidewalk"), "webpage_activity") {
  ...
  def * = (webpageActivityId, userId, activity, timestamp) <> ((WebpageActivity.apply _).tupled, WebpageActivity.unapply)
}
