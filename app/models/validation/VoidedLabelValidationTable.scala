...
/**
 * A validation voided by the #4842 off-target-markers repair (evolution 353): a full copy of a deleted
...
class VoidedLabelValidationTableDef(tag: slick.lifted.Tag)
  ...
  // by evolution 353's Downs to regenerate that row byte-identically. Deliberately outside the default projection:
