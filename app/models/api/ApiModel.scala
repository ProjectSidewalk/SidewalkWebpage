 ...
 * @param maxSeverity Optional maximum severity score (1-5 scale)
 */
case class LabelClusterFilters(
  bbox: Option[LatLngBBox] = None,
  labelTypes: Option[Seq[String]] = None,
  regionId: Option[Int] = None,
  regionName: Option[String] = None,
 ...
 * @param imageCaptureDate Optional date when the Street View image was captured
 */
case class RawLabelInClusterData(
  labelId: Int,
  userId: String,
  gsvPanoramaId: String,
  severity: Option[Int],
...
/**
 * Companion object for RawLabelInClusterData containing JSON formatter
 */
object RawLabelInClusterData {
  implicit val clusterLabelDataWrites: Writes[RawLabelInClusterData] = Json.writes[RawLabelInClusterData]
}
...
case class LabelClusterForApi(
  ...
  clusterSize: Int,
  userIds: Seq[String],
  labels: Option[Seq[RawLabelInClusterData]],
  avgLatitude: Double,
  avgLongitude: Double,
) extends StreamingApiType {
