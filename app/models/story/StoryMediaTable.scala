 ...
 * @param captureRecency Coarse age bucket derived transiently from upload metadata; the precise timestamp is discarded.
 * @param nearLabel      Whether upload GPS metadata placed the capture near the label; the precise fix is discarded.
...
class StoryMediaTableDef(tag: Tag) extends Table[StoryMedia](tag, "story_media") {
  ...
  def * : ProvenShape[StoryMedia] =
    (storyMediaId, storyId, mediaType, mimeType, width, height, durationSecs, fileSizeBytes, altText, captureRecency,
      nearLabel, createdAt) <> ((StoryMedia.apply _).tupled, StoryMedia.unapply)
}
