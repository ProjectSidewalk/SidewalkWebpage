 ...
 *                          a label past the end of its edge keeps only its cross-track component (see 377.sql).
...
class LabelPointTable @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit
   ...
   * follows. Calls the same SQL function as 377.sql's backfill, so a stored value always equals a fresh recompute.
   *
