...
@Singleton
class UserClusteringSessionTable @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit
    ec: ExecutionContext
) extends UserClusteringSessionTableRepository
  ...
  def labelsForApiQuery: Query[
    (Rep[String], Rep[Int], Rep[String], Rep[Float], Rep[Float], Rep[Option[Int]]),
    (String, Int, String, Float, Float, Option[Int]),
    Seq
  ] = for {
  ...
  } yield (_mission.userId, _lab.labelId, _type.labelType, _latlng.lat.ifNull(-1f), _latlng.lng.ifNull(-1f),
    _lab.severity)
  ...
  def getClusteredLabelsInRegion(regionId: Int): DBIO[Seq[LabelToCluster]] = {
    ...
    } yield (_sess.userId, _att.userAttributeId, _type.labelType, _att.lat, _att.lng, _att.severity)).result
      .map(_.map(LabelToCluster.tupled))
  }
