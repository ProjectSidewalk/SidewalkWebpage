...
object AuditTaskTable {
  ...
  implicit val newTaskConverter = GetResult[NewTask](r => {
    NewTask(r.nextInt, r.nextGeometry[LineString], r.nextFloat, r.nextFloat, r.nextFloat, r.nextFloat, r.nextFloat,
      r.nextFloat, r.nextString, r.nextBoolean, r.nextTimestamp, r.nextBoolean, r.nextDouble,
  ...
  def selectANewTask(streetEdgeId: Int, missionId: Int, reverseStartPoint: Boolean = false, routeStreetId: Option[Int] = None): NewTask = db.withSession { implicit session =>
    ...
    } yield (se.streetEdgeId, se.geom, if (reverseStartPoint) se.x2 else se.x1, if (reverseStartPoint) se.y2 else se.y1, se.x1, se.y1, se.x2, se.y2, se.wayType, reverseStartPoint, timestamp, scau._2, sep.priority, false, None: Option[Int], Some(missionId).asColumnOf[Option[Int]], None: Option[Point], routeStreetId)
  ...
  def getATutorialTask(missionId: Int): NewTask = db.withSession { implicit session =>
    ...
    val tutorialTask = streetEdges
      ...
      .map(e => (e.streetEdgeId, e.geom, e.x1, e.y1, e.x1, e.y1, e.x2, e.y2, e.wayType, false, timestamp, false, 1.0, false, None: Option[Int], missionId.asColumnOf[Option[Int]], None: Option[Point], None: Option[Int]))
  ...
  def selectANewTaskInARegion(regionId: Int, user: UUID, missionId: Int): Option[NewTask] = db.withSession { implicit session =>
    ...
    } yield (se.streetEdgeId, se.geom, se.x1, se.y1, se.x1, se.y1, se.x2, se.y2, se.wayType, false, timestamp, sc._2, sp.priority, false, None: Option[Int], Some(missionId).asColumnOf[Option[Int]], None: Option[Point], None: Option[Int])
  ...
  def selectTaskFromTaskId(taskId: Int, routeStreetId: Option[Int] = None): Option[NewTask] = db.withSession { implicit session =>
    ...
    } yield (se.streetEdgeId, se.geom, at.currentLng, at.currentLat, se.x1, se.y1, se.x2, se.y2, se.wayType, at.startPointReversed, at.taskStart, sc._2, sp.priority, at.completed, at.auditTaskId.?, at.currentMissionId, at.currentMissionStart, routeStreetId)
  ...
  def selectTasksInARegion(regionId: Int, user: UUID): List[NewTask] = db.withSession { implicit session =>
    ...
    } yield (se.streetEdgeId, se.geom, se.x1, se.y1, se.x1, se.y1, se.x2, se.y2, se.wayType, false, ucs._2.?.getOrElse(timestamp), scau._2, sep.priority, ucs._1.?.isDefined, ucs._3.?, ucs._4, ucs._5, None: Option[Int])
