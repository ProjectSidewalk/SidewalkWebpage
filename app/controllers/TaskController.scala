...
class TaskController @Inject() (implicit val env: Environment[User, SessionAuthenticator])
  ...
  def getTaskByStreetEdgeId(streetEdgeId: Int) = Action.async { implicit request =>
    AuditTaskTable.selectANewTask(streetEdgeId, None).map { task =>
      Ok(task.toJSON)
    }
  }
  ...
  def getTasksInARegion(regionId: Int) = UserAwareAction.async { implicit request =>
    request.identity match {
      case Some(user) =>
        AuditTaskTable.selectTasksInARegion(regionId, user.userId).map { tasks =>
          Ok(JsArray(tasks.map(_.toJSON)))
        }
      case None =>
        AuditTaskTable.selectTasksInARegion(regionId).map { tasks =>
          Ok(JsArray(tasks.map(_.toJSON)))
        }
    }
  }
   ...
   * @return
   */
  def updateAuditTaskTable(user: Option[User], auditTask: TaskSubmission, amtAssignmentId: Option[Int]): Future[Int] = {
   ...
   */
  def updateMissionTable(user: Option[User], missionProgress: AuditMissionProgress): Future[Option[Mission]] = {
    ...
    (for {
      regionId <- UserCurrentRegionTable.currentRegion(userId)
      isOnboardingMission <- MissionTable.isOnboardingMission(missionProgress.missionId)
    } yield (regionId.get, isOnboardingMission)).flatMap {
      case (regionId, isOnboardingMission) =>
        val missionFuture: Future[Option[Mission]] =
          if (isOnboardingMission) {
            if (missionProgress.completed) {
              MissionTable.updateCompleteAndGetNextMission(userId, regionId, payPerMeter, missionId, skipped)
            } else {
            ...
            if (missionProgress.completed) {
              MissionTable.updateCompleteAndGetNextMission(userId, regionId, payPerMeter, missionId, distProgress, skipped)
            } else {
  ...
  def updateAuditTaskCompleteness(auditTaskId: Int, auditTask: TaskSubmission, incomplete: Option[IncompleteTaskSubmission]): Future[Any] = {
    ...
    if ((auditTask.completed.isDefined && auditTask.completed.get)
      ...
      AuditTaskTable.anyoneHasAuditedStreet(auditTask.streetEdgeId).flatMap {
        case true => Future.successful(Nil)
        case false => RegionCompletionTable.updateAuditedDistance(auditTask.streetEdgeId)
      }.flatMap { _ =>
