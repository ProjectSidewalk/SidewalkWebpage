...
class UserProfileController @Inject() (implicit val env: Environment[User, SessionAuthenticator])
   ...
   */
  def getAllAuditedStreets(filterLowQuality: Boolean, notAudited: Boolean) = UserAwareAction.async { implicit request =>
    val streets = if (notAudited) {
      AuditTaskTable.selectStreetsNotAudited();
    } else {
      AuditTaskTable.selectStreetsAudited(filterLowQuality);
    }
    val features: List[JsObject] = streets.map { edge =>
      val coordinates: Array[Coordinate] = edge.geom.getCoordinates
