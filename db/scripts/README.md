...
| `allow-mapillary-creators.sh` | (called by `make onboard-city`) | Seeds `mapillary_allowed_source`, restricting a Mapillary city to imagery from the given creators (#5407). Idempotent. | Onboarding a city that must launch already restricted; on a running deployment use `/admin/imagery`, which checks each username against Mapillary first. |
