...
- **Coverage can block too.** `Backend tests (API, PostGIS)` ends on a statement-coverage ratchet, so removing tests
  can fail the build even when everything still passes. The JS suite has a floor of its own, but it sits in an
  advisory step, so a dip there reports without blocking.
