...
L.Edit.Poly.include( /** @lends L.Edit.Poly.prototype */ {
  ...
  _onStartDragFeature: function(evt) {
    // KH: Need to edit this so that connected nodes in other layers too.
    var m = this._poly._map;
    var thisPoly = this._poly;
    var polyLatLngs = thisPoly.getLatLngs();
    ...
    m.eachLayer(function (layer) {
      if(layer instanceof L.Polyline) {
        if (layer === thisPoly) {
          ...
          // Brute force
          for (var polyIdx = 0; polyIdx < polyLatLngs.length; polyIdx++) {
            for (var layerIdx = 0; layerIdx < polyLatLngs.length; layerIdx++) {
              if (layerLatLngs[layerIdx]) {
                var margin = Math.max(
                    Math.abs(polyLatLngs[polyIdx].lat - layerLatLngs[layerIdx].lat),
                    Math.abs(polyLatLngs[polyIdx].lng - layerLatLngs[layerIdx].lng));
                var equals = margin < L.LatLng.MAX_MARGIN;
                if (equals) {
                  console.log("Margin, Overlaps:", margin, equals);
                }
