#!/usr/bin/env python3
"""Regenerates test/e2e/fixtures/ci-seed.sql from a prod slice pulled by test/e2e/fixtures/ci_seed_slice.sql (#5115).

    python3 test/e2e/fixtures/gen_ci_seed.py [path/to/ci-seed-slice.csv]
...
Getting the slice (the maintainer runs this; see CLAUDE.local.md):
    ...
    scp test/e2e/fixtures/ci_seed_slice.sql saugstad@makelab1.cs.washington.edu:sidewalk-server-tools/current-query.sql
...
w("""-- Every street needs an OSM way, because /v3/api/rawLabels and the label-clusters API both INNER JOIN
...
FROM (VALUES """ + ", ".join(f"({s})" for s in sorted(STREETS)) + """) AS s(street_edge_id)
WHERE NOT EXISTS (
