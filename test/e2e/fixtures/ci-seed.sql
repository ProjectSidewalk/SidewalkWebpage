...
-- streets, labels and panoramas -- regenerated by scratchpad/gen-ci-seed.py from scratchpad/ci-seed-slice.sql (#5115).
-- Real, because a fixture that invents its own coordinates and panorama ids can only show that the code runs, not
-- that it runs on the shape of data it will meet.
--
...
VALUES
  ...
  (900032, 900004, 900001, '00000000-5115-4000-8000-000000000002', '60Fy6udfNJjDILZZ79n7Bg', 4, 1334, 11,
   now() - INTERVAL '26 days', FALSE, FALSE, 1, '{"bumpy"}'),  -- SurfaceProblem, prod label 21798
  (900033, 900003, 900002, '00000000-5115-4000-8000-000000000003', 'gQIxyDH1bxP1In5JOau-mg', 4, 1141, 7,
   now() - INTERVAL '24 days', FALSE, FALSE, 1, '{"utility panel"}'),  -- SurfaceProblem, prod label 24725
  -- The Mapillary pano's label, first so that it is not the NEWEST: the share and story specs read the
  -- highest label id, and half of what they assert needs one whose pano carries GSV metadata.
  (900001, 900003, 900002, '00000000-5115-4000-8000-000000000003', '1050859256685844', 4, 1141, 99,
...
SELECT p.* FROM (VALUES
  (900002, 900002, 10900, 4853, 164, 333,
  ...
  (900008, 900008, 9589, 5055, 643, 285,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900009, 900009, 6778, 5040, 196, 230,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900010, 900010, 6243, 4934, 263, 189,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900011, 900011, 5844, 4624, 143, 106,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900012, 900012, 4892, 5193, 176, 271,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900013, 900013, 5843, 5189, 309, 252,
  ...
  (900015, 900015, 1623, 4634, 415, 297,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900016, 900016, 9624, 4997, 481, 209,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900017, 900017, 6756, 5037, 254, 237,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900018, 900018, 6344, 4950, 191, 232,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900019, 900019, 1857, 4945, 532, 346,
  ...
  (900021, 900021, 8140, 5261, 258, 325,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900022, 900022, 6197, 4787, 248, 148,
  ...
  (900025, 900025, 2391, 4706, 209, 246,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900026, 900026, 5575, 4718, 366, 267,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900027, 900027, 2245, 4668, 415, 152,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900028, 900028, 5924, 4647, 406, 103,
  ...
  (900030, 900030, 2325, 4711, 414, 163,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900031, 900031, 15116, 4490, 280, 143,
  ...
  (900033, 900033, 10654, 4586, 367, 350,
   ...
   'depth'::sidewalk_teaneck.computation_method),
  (900001, 900001, 6777, 3087, 361, 214,
...
VALUES (900004, 'validation', '00000000-5115-4000-8000-000000000003',
...
VALUES (900001, 900002, 'Agree', '00000000-5115-4000-8000-000000000003', 900004, 300, 200, 120.0, -10.0, 1.0,
...
INSERT INTO sidewalk_teaneck.validation_task_comment (validation_task_comment_id, mission_id, label_id, user_id,
                                                      ...
                                                      timestamp, comment)
VALUES (900001, 900004, 900002, '00000000-5115-4000-8000-000000000003', '127.0.0.1', 'gQIxyDH1bxP1In5JOau-mg',
...
-- auditOnboarding), deleted ones included, minus anything on the tutorial street.
UPDATE sidewalk_teaneck.user_stat
...
-- The quality heuristic, minus the branches these rows can't reach: none is excluded or manually rated, none has an
-- accuracy or 50 validated labels, so it reduces to the labeling-frequency test against its 0.0375 threshold. The
-- two seeded labelers land either side of it, which exercises the recompute in both directions.
UPDATE sidewalk_teaneck.user_stat
