 ...
 * data-pano-scoring stamp the pano viewer reads its ranking weights back from (#4411).
 *
...
class MobileDetectionSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  "The layout's data-pano-scoring stamp" should {
    ...
    "carry the parsed scoring parameters on any page" in {
      val resp = route(app, FakeRequest(GET, "/signIn")).get
      status(resp) mustBe OK
      val stamp: String = "data-pano-scoring=\"([^\"]*)\"".r
        .findFirstMatchIn(contentAsString(resp))
        .map(_.group(1))
        .getOrElse(fail("the layout did not stamp data-pano-scoring"))
      Json.parse(stamp.replace("&quot;", "\"")) mustBe Json.parse(models.utils.PanoScoring.json)
    }
  }
