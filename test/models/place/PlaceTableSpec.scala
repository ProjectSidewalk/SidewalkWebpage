 ...
 * a refresh. Also holds evolution 399's category CHECK to the Scala catalog, so a category added to one is missed by
...
class PlaceTableSpec extends PlaySpec with GuiceOneAppPerSuite with RolledBackDb with StreetFixtures with OptionValues {
  ...
  "the category CHECK in evolution 399" should {
    "list exactly the catalog's ids, in its order" in {
      val script = {
        val source = Source.fromFile("conf/evolutions/default/399.sql", "UTF-8")
        try source.mkString
