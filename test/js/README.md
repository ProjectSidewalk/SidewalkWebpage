...
- `access-score/src/AccessScoreModel.js` → `accessScoreModel.test.js` — the AccessScore tool's in-browser scoring
  model (#5217), held to the same committed fixture as the Scala engine (`test/fixtures/accessScoreParity.json`,
  regenerated by `sbt "Test/runMain service.AccessScoreParityFixtureGen"` and pinned server-side by
...
- `explore/src/navigation/ForwardCrumbs.js` → `forwardCrumbsLifecycle.test.js` — the stateful half, driven through
  `refresh()`/`clear()`/click with a stubbed `Minimap.addMarker` and a scripted provider: a stop that renumbers gets
