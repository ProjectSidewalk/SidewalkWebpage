...
class SharedLabelPage {
    ...
    async #initStats() {
        ...
        try {
            ...
            const values = {
                ...
                miles: s.km_explored_no_overlap != null ? Math.round(s.km_explored_no_overlap * KM_TO_MI) : null
            };
            let any = false;
    ...
    /**
     * Builds the nearby-labels minimap: a base PSMap centered on this label, plus label markers fetched from the cheap
    ...
    async #initMap() {
        ...
        if (!holder || d.lat == null || d.lng == null) return; // No location → the section isn't rendered.
    ...
    #wireNearbyPopups(map, mapData) {
        ...
        const popup = new mapboxgl.Popup({ closeButton: true, closeOnClick: true, offset: 12, className: 'spotlight-popup' });
        ...
        map.on('mouseenter', layerNames, () => { map.getCanvas().style.cursor = 'pointer'; });
        map.on('mouseleave', layerNames, () => { map.getCanvas().style.cursor = ''; });
    }
