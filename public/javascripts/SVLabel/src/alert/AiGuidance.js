    ...
    if (window.cityId === 'chandigarh-india') {
      this.#analyzeCurrentStreetWithGemini(svl.taskContainer.getCurrentTask())
        .then((response) => {
          ...
          const body = `<span>${response}  <span title="This was generated by AI using imagery from this street." `
            + `style="cursor: pointer">&#9432;</span></span>`;

          // Show the pop-up message with the AI guidance.
          this.#popUpMessage.notify(title, body, () => {
            // Log the message shown to the user.
            this.#tracker.push('PopUpShow_AiGuidance', { message: response });
          });
