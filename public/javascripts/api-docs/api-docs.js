...
/**
 * Finds related static nav items (e.g., 'page' and 'page#section') and groups
 * them into an accordion structure in the HTML.
...
/**
 * Finds the primary active nav item, scans content for H1/H2,
 * and generates an expanded submenu structure in the HTML.
 * Does NOT add event listeners here.
 */
...
/**
 * Sets up a single event listener on the navigation container
 * to handle clicks on all accordion triggers using event delegation.
 */
  ...
  navContainer.addEventListener('click', function(event) {
    ...
    if (accordionTrigger) {
      ...
      if (accordionTrigger.tagName === 'A' && accordionTrigger.getAttribute('href')) {
           // Check if the click was directly on the trigger or its arrow,
           // not on a link *inside* a submenu that might bubble up.
...
/**
 * Sets up scroll spy to highlight active TOC items based on scroll position.
 * Left sidebar H1/H2 links are NOT dynamically highlighted by scroll spy.
 * @returns {void}
 */
...
/**
 * Sets up smooth scrolling for TOC and sidebar hash links.
 * Does NOT update left sidebar highlighting on click.
 * @returns {void}
 */
