var sampleLabels = {
	sampleSet1: {	
		features: [{
	geometry: {
		coordinates: [
			-77.04338836669922,
			38.90653991699219
		],
		type: "Point"
	},
	properties: {
		audit_task_id: 4306,
		gsv_panorama_id: "tQCMMo25nchU87fdnJGmmQ",
		label_id: 20232,
		label_type: "NoCurbRamp",
		severity: 3
	},
	type: "Feature"
			},
			{
				geometry: {
					coordinates: [
						-77.04351043701172,
						38.90653610229492
					],
					type: "Point"
				},
				properties: {
					audit_task_id: 4306,
					gsv_panorama_id: "tQCMMo25nchU87fdnJGmmQ",
					label_id: 20233,
					label_type: "NoCurbRamp",
					severity: 3
				},
				type: "Feature"
			},
			{
				geometry: {
					coordinates: [
						-77.04351043701172,
						38.906402587890625
					],
					type: "Point"
				},
				properties: {
					audit_task_id: 4306,
					gsv_panorama_id: "LyhDWKVOM1keWutriZGFqA",
					label_id: 20239,
					label_type: "NoCurbRamp",
					severity: 3
				},
				type: "Feature"
			},
			{
				geometry: {
					coordinates: [
						-77.04337310791016,
						38.90642547607422
					],
					type: "Point"
				},
				properties: {
					audit_task_id: 4306,
					gsv_panorama_id: "GkI7OXvuFkXt53ys0BWD6g",
					label_id: 20234,
					label_type: "CurbRamp",
					severity: 1
				},
				type: "Feature"
			},
			{
				geometry: {
					coordinates: [
						-77.04337310791016,
						38.906410217285156
					],
					type: "Point"
				},
				properties: {
					audit_task_id: 4306,
					gsv_panorama_id: "bT-OJVyH3HsmoUh700xUpw",
					label_id: 20228,
					label_type: "CurbRamp",
					severity: 1
				},
				type: "Feature"
			}
		],
		type: "FeatureCollection"
	},
	sampleSet2: {
		features: [
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"rb3UJINCRDFJwbPzKhOFyg",
					label_id:7823,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03860473632812, 38.912681579589844]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:19536,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03857421875,38.912689208984375]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"ncHz5AFhGssO2GB6QIsosA",
					label_id:7805,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.0385513305664,38.91270065307617]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"rb3UJINCRDFJwbPzKhOFyg",
					label_id:7822,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.0385971069336,38.91253662109375]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:19535,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03857421875,38.91255187988281]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"7BXtxjBQQeQESx0OIQvcRg",
					label_id:7828,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03854370117188,38.91252136230469]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:7827,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03838348388672,38.91253662109375]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:7826,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03833770751953,38.91256332397461]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:1693,
					gsv_panorama_id:"rb3UJINCRDFJwbPzKhOFyg",
					label_id:7825,
					label_type:"CurbRamp",
					severity:1
				},
				geometry: {
					coordinates: [-77.03840637207031,38.91268539428711]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:19538,
					label_type:"CurbRamp",
					severity:3
				},
				geometry: {
					coordinates: [-77.03837585449219,38.91269302368164]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"IhGTPdLdCI0xjErLEucW6g",
					label_id:19541,
					label_type:"Obstacle",
					severity:1
				},
				geometry: {
					coordinates: [-77.0383529663086,38.91265106201172]
				}
			},
			{
				type: "Feature",
				properties: {
					audit_task_id:4168,
					gsv_panorama_id:"vxMmytN--JycF7nGKPUBvQ",
					label_id:19539,
					label_type:"CurbRamp",
					severity:2
				},
				geometry: {
					coordinates: [-77.03833770751953,38.91267776489258]
				}
			}
		],
		type: "FeatureCollection"
	}
}