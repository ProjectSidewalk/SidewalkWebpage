 ...
 * that comes back when told finds the value. The same shape covers a `502`/`504` from the reverse proxy in front of a
 * slow answer. Nothing else is retried: a `4xx` means the request itself is wrong, and a network failure is the
 * page's error card, not a wait.
 */
class AccessScoreFetch {
  ...
  /** How long, in total, the page keeps waiting before it gives up and shows the error card. */
   ...
   * @param {number} [options.maxTotalWaitSeconds] - Cap on the summed waits; defaults to five minutes.
   * @returns {Promise<any>} The parsed JSON of the first successful response.
   * @throws {Error} The last failure, once a response is not retryable or the cap would be exceeded.
   */
  static async fetchJsonWithRetry(url, { onWait, isRetryable, maxTotalWaitSeconds } = {}) {
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      // Giving up is a decision about the total, not the count: a server naming long waits runs the cap down faster.
      if (waitedSeconds + delaySeconds > cap) throw error;
